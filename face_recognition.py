...
class Face_Recognition:
    def __init__(self,root):
        ...
        img_left=Image.open("face_recognition1.jpg")
        img_left=img_left.resize((700,700),Image.ANTIALIAS)
        self.photoimg_left=ImageTk.PhotoImage(img_left)

        f_lbl=Label(self.root,image=self.photoimg_left)
        f_lbl.place(x=0,y=45,width=700,height=700)

        img_right=Image.open("face_recognition2.jpg")
        img_right=img_right.resize((700,700),Image.ANTIALIAS)
        self.photoimg_right=ImageTk.PhotoImage(img_right)

        f_lbl=Label(self.root,image=self.photoimg_right)
        f_lbl.place(x=700,y=45,width=700,height=700)
        ...
        b1_1.place(x=245,y=612,width=200,height=40)
    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for(x,y,w,h) in features:
                ...
                if confidence>77:
                    cv2.putText(img,f"Student Id:{i}",(x,y-75),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,255,0),3)
                    cv2.putText(img,f"RollNo:{r}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,255,0),3)
        ...
        #video_cap=cv2.imread(r"F:\attendance_system_using_face_recognition\chetana.jpeg")
        #suraj
        #address="http://192.168.43.243:8080/video"
        #chetana
        #address="http://25.241.196.120:8080/video"
        #prajakta
        #address="http://10.147.38.116:8080/video"
